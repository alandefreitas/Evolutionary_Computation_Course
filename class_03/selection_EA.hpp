...
#include "selection_EA.h"

template <typename problem, typename solution>
std::default_random_engine selection_EA<problem,solution>::_generator = std::default_random_engine(std::chrono::system_clock::now().time_since_epoch().count());

template <typename problem, typename solution>
selection_EA<problem,solution>::selection_EA(problem &p) :
    _problem(p),
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::initialize_population(){
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::run() {
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::run(size_t iterations) {
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::evolutionary_cycle() {
    display_status();
    evaluation_step();
...
template <typename problem, typename solution>
double selection_EA<problem,solution>::best_fx() {
    return this->_best_fx;
}

template <typename problem, typename solution>
void selection_EA<problem,solution>::evaluate(population_type& population){
...
template <typename problem, typename solution>
size_t selection_EA<problem,solution>::n_of_selection_candidates(){
...
template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::selection(population_type& population, size_t n_of_candidates, selection_strategy s){
    switch (s){
...
template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::reproduction(population_type& parents){
    ...
    for (int j = 0; j < parents.size(); j += this->_parents_per_children) {
        if (r(selection_EA::_generator) < this->_crossover_probability) {
            children.emplace_back(
                    std::make_shared<individual>(
                        parents[j]->s.crossover(
                            this->_problem,
                            parents[j+1]->s
                        )
                    )
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::display_status() {
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::scaling(population_type& population, scaling_strategy s) {
    switch (s){
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::attribute_fitness_from_rank(selection_EA::population_type &population) {
    std::sort(population.begin(),population.end(),
              [](individual_ptr& a, individual_ptr &b){
                  return a->fx < b->fx;
              });
...
template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::uniform_selection(selection_EA::population_type &population, size_t n_of_candidates) {
    ...
    for (individual_ptr& parent : parents) {
        parent = population[pos_d(selection_EA::_generator)];
    }
    return parents;
...
template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::tournament_selection(selection_EA::population_type &population, size_t n_of_candidates) {
    population_type parents;
    std::uniform_int_distribution<size_t> pos_d(0,population.size()-1);
    for (int i = 0; i < n_of_candidates; ++i) {
        size_t position = pos_d(selection_EA::_generator);
        for (int j = 1; j < this->_tournament_size; ++j) {
            size_t position2 = pos_d(selection_EA::_generator);
...
template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::roulette_selection(selection_EA::population_type &population, size_t n_of_candidates) {
    population_type parents;
    ...
    for (int i = 0; i < n_of_candidates; ++i) {
        parents.push_back(population[pos_d(selection_EA::_generator)]);
    }
    return parents;
}

template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::truncate_selection(selection_EA::population_type &population, size_t n_of_candidates) {
    population_type parents;
    parents.reserve(n_of_candidates);
    ...
    std::shuffle(parents.begin(),parents.end(),selection_EA::_generator);
    return parents;
}

template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::sus_selection(selection_EA::population_type &population, size_t n_of_candidates) {
    population_type parents;
    parents.reserve(n_of_candidates);
    ...
    double r = dist_r(selection_EA::_generator);
...
template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::overselection_selection(selection_EA::population_type &population, size_t n_of_candidates) {
    population_type parents;
    parents.reserve(n_of_candidates);
    std::partial_sort(population.begin(),
    ...
    for (int i = 0; i < n_of_candidates * 0.8; ++i) {
        parents.push_back(population[pos_best(selection_EA::_generator)]);
    }
    ...
    for (int i = 0; i < n_of_candidates - (n_of_candidates * 0.8); ++i) {
        parents.push_back(population[pos_worst(selection_EA::_generator)]);
    }
    return parents;
}

template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::roundrobin_selection(selection_EA::population_type &population, size_t n_of_candidates) {
    ...
    for (int i = 0; i < population.size(); ++i) {
        ...
        for (int j = 1; j < this->_roundrobin_tournament_size; ++j) {
            solution_with_score& player2 = scores[pos_d(selection_EA::_generator)];
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::window_scaling(selection_EA::population_type &population) {
    for (individual_ptr& ind : population) {
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::sigma_scaling(selection_EA::population_type &population) {
    for (individual_ptr& ind : population) {
        ind->fitness = this->_problem.is_minimization() ? -ind->fx : ind->fx;
    }
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::linear_rank_scaling(selection_EA::population_type &population) {
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::exponential_rank_scaling(selection_EA::population_type &population) {
    attribute_fitness_from_rank(population);
    for (individual_ptr& ind : population) {
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::evaluation_step() {
    evaluate(this->_population);
    scaling(this->_population,scaling_strategy::exponential_rank);
}

template <typename problem, typename solution>
typename selection_EA<problem,solution>::population_type selection_EA<problem,solution>::reproduction_step() {
...
template <typename problem, typename solution>
void selection_EA<problem,solution>::population_update_step(population_type& children) {
    // update population
